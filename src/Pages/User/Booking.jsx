...
//   // Check for pending reschedule requests - FIXED VERSION
//   const checkRescheduleNotifications = async () => {
//     try {
//       const res = await apiConnector("GET", bookingEndpoints.MY_BOOKINGS);
//       if (res.data.success) {
//         const now = new Date();
//         const upcomingBookings = res.data.upcoming || [];
        
//         // Filter only bookings that are actually in the future
//         const pendingRequests = upcomingBookings.filter(booking => {
//           // Check if booking is CONFIRMED and has pending reschedule from mentor
...
//           // Check if the original session time has already passed
//           if (booking.meetingStartTime) {
//             try {
//               const sessionTime = new Date(booking.meetingStartTime);
//               // If original session time has passed, don't show as pending
//               return sessionTime > now;
//             } catch (e) {
//               console.warn("Invalid meetingStartTime:", booking.meetingStartTime);
//               return false; // If can't parse date, don't show
//             }
//           }
          
//           // Fallback: check slot date/time
...
//               }
//             } catch (e) {
//               console.warn("Invalid slot time:", e);
//             }
//           }
          
//           return false; // Default to not showing if we can't determine time
//         });
...
//     // For upcoming tab, filter out bookings that have already passed
...
//           } catch (e) {
//             console.warn("Invalid meetingStartTime:", booking.meetingStartTime);
//             return true; // Keep it if we can't parse
//           }
//         }
        
//         // Fallback to slot time
//         if (booking.slots && booking.slots.length > 0) {
//           const slot = booking.slots[0];
//           if (slot.date && slot.startTime) {
//             try {
...
//         try {
//           const sessionTime = new Date(booking.meetingStartTime);
//           setIsTimePassed(sessionTime <= now);
//           return;
//         } catch (e) {
//           console.warn("Invalid meetingStartTime:", booking.meetingStartTime);
//         }
...
//   // Don't show reschedule button if time has passed
//   const shouldShowRescheduleButton = hasPendingReschedule && isUpcoming && !isTimePassed;

//   return (
//     <div className={`group bg-white/5 border rounded-xl p-5 hover:bg-white/10 transition-all duration-300 hover:scale-[1.01] hover:shadow-xl ${
//       hasPendingReschedule && !isTimePassed
//         ? "border-amber-500/50 animate-pulse" 
//         : isTimePassed
...
//     }`}>
//       {/* Time Passed Warning */}
//       {isTimePassed && isUpcoming && (
//         <div className="mb-3 flex items-center justify-between px-3 py-1.5 rounded-full bg-gray-500/20">
//           <div className="flex items-center gap-2">
...
//             {booking.slots[0]?.date ? formatDate(booking.slots[0].date) : ''}
//           </span>
//         </div>
//       )}

//       {/* Pending Reschedule Badge - only show if time hasn't passed */}
//       {hasPendingReschedule && !isTimePassed && (
//         <div className="mb-3 flex items-center justify-between">
//           <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-gradient-to-r from-amber-500/20 to-orange-500/20">
//             <RotateCcw size={14} className="text-amber-400" />
...
//                   <p className="font-medium">
//                     {formatDate(booking.slots[0]?.date)}
//                     {isTimePassed && <span className="text-xs text-gray-500 ml-2">(Passed)</span>}
//                   </p>
//                 </div>
//               </div>
...
//         {/* Right Section - Actions */}
//         <div className="flex flex-col gap-3">
//           {isUpcoming && booking.status === "PENDING" && !isTimePassed && (
//             <div className="space-y-2">
//               <button className="flex items-center justify-center gap-2 bg-gradient-to-r from-yellow-600 to-orange-600 hover:from-yellow-700 hover:to-orange-700 px-4 py-2.5 rounded-lg font-medium transition-all duration-300 hover:scale-105 active:scale-95 w-full">
//                 <CheckCircle size={16} />
//                 Confirm Payment
...
//           {/* Reschedule Button - only show if time hasn't passed */}
...
//           {/* Show expired message if time has passed */}
//           {isTimePassed && isUpcoming && (
//             <div className="space-y-2">
//               <div className="px-4 py-2.5 rounded-lg bg-gray-500/20 border border-gray-500/30 text-center">
//                 <p className="text-sm text-gray-300 font-medium">Session Expired</p>
